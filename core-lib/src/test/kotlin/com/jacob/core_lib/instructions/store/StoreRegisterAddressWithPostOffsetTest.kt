...
internal class StoreRegisterAddressWithPostOffsetTest {
    @Test
    internal fun `reads and updates correct memory address when given positive offset value`() {
        ...
        val loadInstruction = StoreRegisterAddressWithPostOffset(sourceRegister, destinationRegister, offset)
        loadInstruction.execute(executionEnvironment)

        memoryArray.getWordAt(6.MA) `should be equal to` 10.W
        registerArray.getRegisterAt(0.DR).getRegisterValue() `should be equal to` 9.W
    }
    ...
    internal fun `reads and updates correct memory address when given negative offset value`() {
        ...
        memoryArray.getWordAt(6.MA) `should be equal to` 10.W
        registerArray.getRegisterAt(0.DR).getRegisterValue() `should be equal to` 3.W
    }
}
